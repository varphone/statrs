...
mod test {
    ...
    fn test_result<F>(mean: f64, std_dev: f64, expected: f64, eval: F)
        where F : Fn(LogNormal) -> result::Result<f64> {
    ...
    #[test]
    fn test_cdf() {
        test_result(-0.1, 0.1, 0.0, |x| x.cdf(-0.1));
        test_result_almost(-0.1, 0.1, 0.0, 1e-107, |x| x.cdf(0.1));
        test_result_almost(-0.1, 0.1, 0.0000000015011556178148777579869633555518882664666520593658, 1e-19, |x| x.cdf(0.5));
        test_result_almost(-0.1, 0.1, 0.10908001076375810900224507908874442583171381706127, 1e-11, |x| x.cdf(0.8));
        test_result_almost(-0.1, 1.5, 0.070999149762464508991968731574953594549291668468349, 1e-11, |x| x.cdf(0.1));
        test_result(-0.1, 1.5, 0.34626224992888089297789445771047690175505847991946, |x| x.cdf(0.5));
        test_result(-0.1, 1.5, 0.46728530589487698517090261668589508746353129242404, |x| x.cdf(0.8));
        test_result_almost(-0.1, 2.5, 0.18914969879695093477606645992572208111152994999076, 1e-10, |x| x.cdf(0.1));
        test_result(-0.1, 2.5, 0.40622798321378106125020505907901206714868922279347, |x| x.cdf(0.5));
        test_result(-0.1, 2.5, 0.48035707589956665425068652807400957345208517749893, |x| x.cdf(0.8));
        test_result_almost(1.5, 0.1, 0.0, 1e-315, |x| x.cdf(0.1));
        test_result_almost(1.5, 0.1, 0.0, 1e-106, |x| x.cdf(0.5));
        test_result_almost(1.5, 0.1, 0.0, 1e-66, |x| x.cdf(0.8));
        test_result_almost(1.5, 1.5, 0.005621455876973168709588070988239748831823850202953, 1e-12, |x| x.cdf(0.1));
        test_result_almost(1.5, 1.5, 0.07185716187918271235246980951571040808235628115265, 1e-11, |x| x.cdf(0.5));
        test_result_almost(1.5, 1.5, 0.12532699044614938400496547188720940854423187977236, 1e-11, |x| x.cdf(0.8));
        test_result_almost(1.5, 2.5, 0.064125647996943514411570834861724406903677144126117, 1e-11, |x| x.cdf(0.1));
        test_result_almost(1.5, 2.5, 0.19017302281590810871719754032332631806011441356498, 1e-10, |x| x.cdf(0.5));
        test_result_almost(1.5, 2.5, 0.24533064397555500690927047163085419096928289095201, 1e-16, |x| x.cdf(0.8));
        test_result(2.5, 0.1, 0.0, |x| x.cdf(0.1));
        test_result_almost(2.5, 0.1, 0.0, 1e-223, |x| x.cdf(0.5));
        test_result_almost(2.5, 0.1, 0.0, 1e-162, |x| x.cdf(0.8));
        test_result_almost(2.5, 1.5, 0.00068304052220788502001572635016579586444611070077399, 1e-13, |x| x.cdf(0.1));
        test_result_almost(2.5, 1.5, 0.016636862816580533038130583128179878924863968664206, 1e-12, |x| x.cdf(0.5));
        test_result_almost(2.5, 1.5, 0.034729001282904174941366974418836262996834852343018, 1e-11, |x| x.cdf(0.8));
        test_result_almost(2.5, 2.5, 0.027363708266690978870139978537188410215717307180775, 1e-11, |x| x.cdf(0.1));
        test_result_almost(2.5, 2.5, 0.10075543423327634536450625420610429181921642201567, 1e-11, |x| x.cdf(0.5));
        test_result_almost(2.5, 2.5, 0.13802019192453118732001307556787218421918336849121, 1e-11, |x| x.cdf(0.8));
    }
}